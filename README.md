...
**입지 · 상권** — 지표 하나로 지역 순위를 매기고, 다른 지표를 최대 4개까지
옆에 나란히 붙여 본다. 막대마다 중앙값 선이 그어진다.
...
|---|---:|:---:|---|
| 구 · 시 | 56 | O | 넓게 훑기 |
| 동 묶음 | 519 | O | 동 단위로 인구까지 |
| 법정동 | 1,355 | X | 학원·학교만 정밀하게 |

### '동 묶음'이 왜 필요한가

인구 통계는 **행정동**(상계1동) 기준이고 학원 주소는 **법정동**(상계동) 기준인데,
둘은 일대일이 아니다.

```
상계1~10동  →  전부 상계동만 덮음       →  묶음 「상계동」
청운효자동   →  청운동·효자동·신교동 …    →  이 법정동들이 한 묶음
대치1·2·4동 →  대치동·개포동에 걸침      →  「대치동 · 개포동 외 2」
```

얽힌 것끼리 묶어야 인구도 학원도 어느 쪽으로 새지 않는다. 쪼개려면 추정이
들어가므로 하지 않았다. 대신 묶음마다 크기가 고르지 않다 — 행정구역 자체가 그렇다.
...
| 연령별 인구 | 행정안전부 · 주민등록 인구통계 (행정동 × 1세) | 매월 |
| 행정동↔법정동 | 국가데이터처 · 법정동 연계정보 | 분기 |
...
python build/fetch_population.py                 # 행정동별 연령 인구 (56회 호출)
...
- 영어학원 593곳(4.5%)은 주소에 동 정보가 없어 동 묶음에서 빠진다
- 인구 미매칭 행정동 16개 (연계정보 기준일 이후 신설)
